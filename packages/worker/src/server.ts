/**
 * Worker — WebSocket server that wraps @anthropic-ai/claude-agent-sdk.
 *
 * Lightweight Node.js process. Single responsibility: own this machine's
 * Claude sessions and expose them over the wire protocol. No REST — WebSocket
 * only. Accepts commands, manages SdkSession instances, and broadcasts ALL
 * events back (text, tools, questions, plan, compact, permissions, result).
 *
 * ---------------------------------------------------------------------------
...
export class WorkerServer {
  ...
  constructor(private options: WorkerServerOptions) {
  ...
  async start(): Promise<number> {
    ...
    for (const p of persisted) {
      // Don't auto-resume — the client decides when to resume

/**
 * Worker state — minimal persistence for crash recovery.
 *
 * Stores active session IDs + metadata to disk so the worker can mark
 * sessions as idle on restart. Actual resume is handled by the SDK (which
 * stores conversation history in ~/.claude/projects/).
 *
 * ---------------------------------------------------------------------------
